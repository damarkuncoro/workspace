...
module KT::Components::SidebarComponent
  ...
  def build_sidebar_classes(variant:, collapsed:)
    classes = ["kt-sidebar flex flex-col bg-background border-r border-border transition-all duration-300"]

    # Variant
    case variant
    when :compact then classes << "w-16"
    when :standard then classes << "w-64"
    when :wide then classes << "w-80"
    else classes << "w-64"
    end

    # Collapsed state
    classes << "kt-sidebar-collapsed" if collapsed
