# app/helpers/kt/base_ui_helper.rb
module KT::BaseUiHelper
  # ===============================
  # 🔹 SHARED UI PATTERNS
  # ===============================
  ...
  def ui_badge(text:, type: :default, size: :sm, outline: true, badge_class: "kt-badge")
    classes = [badge_class]
    classes << "kt-badge-#{size}"
    classes << "kt-badge-#{type}"
  ...
  def ui_button(text: nil, icon: nil, type: :primary, size: :default, variant: :solid, button_class: "kt-btn", **options)
    ...
    classes << "kt-btn-#{type}"
    classes << "kt-btn-#{size}" unless size == :default
    classes << "kt-btn-#{variant}" unless variant == :solid

    content = []
    content << content_tag(:i, "", class: "ki-filled #{icon}") if icon
    content << text if text

    button_tag(safe_join(content), class: classes.join(" "), **options)
  end
  ...
  def ui_avatar(src:, alt: "", size: "size-9", avatar_class: "kt-avatar", status: nil, status_class: "kt-avatar-status-online")
    content_tag(:div, class: "#{avatar_class} #{size}") do
      ...
      if status
        concat(content_tag(:div, class: "kt-avatar-indicator -end-2 -bottom-2") do
          content_tag(:div, "", class: "kt-avatar-status #{status_class} size-2.5")
        end)
      end
    end
  end
  ...
  def ui_link(text:, href:, link_class: "kt-link", underlined: false, dashed: false)
    classes = [link_class]
    classes << "kt-link-underlined" if underlined
    classes << "kt-link-dashed" if dashed
    link_to(text, href, class: classes.join(" "))
  end

  # ✅ SRP: Generic icon component
  def ui_icon(icon_class:, size: "text-lg", icon_wrapper_class: "")
    wrapper_class = icon_wrapper_class.present? ? icon_wrapper_class : ""
    content_tag(:i, "", class: "#{icon_class} #{size} #{wrapper_class}")
  end
end